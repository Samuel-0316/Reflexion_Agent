...
def actor_query_node(state: dict) -> dict:
    ...
    prompt = (
        ...
        f"- If this is the first attempt, keep it concise: product name + 'price India'.\n"
        f"- **If reflections are provided above, you MUST incorporate their "
        f"suggestions into a meaningfully DIFFERENT query. Do NOT repeat "
        f"the same query as a previous attempt.** Try different phrasing, "
        f"add keywords like 'buy online', 'official store', 'retail price', "
        f"or rearrange the product name.\n\n"
...
def actor_verdict_node(state: dict) -> dict:
    ...
    prompt = (
        ...
        f"actual numeric INR price explicitly attached to the EXACT PRODUCT REQUESTED in the same snippet.**\n"
        f"- **CRITICAL: Do NOT hallucinate prices or mix-and-match prices between sources.** If a snippet does not contain a clear price for the product, do NOT assign it a price from another snippet.\n"
        f"  Do NOT include:\n"
        f"    ✗ Sources with 'No price available' or 'N/A' as the price\n"
        f"    ✗ Sources listing DIFFERENT products, related ads, or different variants (e.g., Rockerz 550 or Rockerz 450R when the user asked for Rockerz 450)\n"
...
def evaluator_node(state: dict) -> dict:
    ...
    prompt = (
        ...
        f"Also, you MUST check the 'name' and 'url' of the sources provided to ensure they do not point to a different variant (e.g. if the user asked for 'boAt Rockerz 450', a source URL containing 'boat-rockerz-450r' or 'boat-rockerz-plus-450' is product drift and must FAIL). "
